...
class PCN(data.Dataset):
    ...
    def _get_transforms(self, subset):
        if subset == 'train':
            ...
            # 不再依赖 random_rotation 决定是否做镜像
            # 镜像始终启用，旋转由 runner.py 在 GPU 侧控制
    ...
    def __getitem__(self, idx):
        ...
        # 仅对测试/验证集且 YAML 配置了 RANDOM_ROTATION 时在 Dataset 侧旋转
        # 训练集的旋转由 runner.py 在 GPU 侧控制（课程学习）
        if self.random_rotation and self.subset != 'train':
